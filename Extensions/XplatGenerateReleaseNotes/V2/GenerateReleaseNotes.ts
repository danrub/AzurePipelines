...
import { Build, Change, BuildQueryOrder, BuildResult } from "vso-node-api/interfaces/BuildInterfaces";
...
async function getChangeset(vsts: webApi.WebApi, buildApi: IBuildApi, teamProject: string, currentBuildId: number, compareBuildId?: number):
    Promise<{ commits: Change[], workitems: ResourceRef[] } | undefined> {
    ...
    if (!compareBuildId) {
        ...
        commits = await buildApi.getBuildChanges(teamProject, currentBuildId);
        workitems = await buildApi.getBuildWorkItemsRefs(teamProject, currentBuildId);
    } else if (currentBuildId !== compareBuildId) {
        agentApi.logInfo(`Checking what commits and workitems have changed from [ID ${compareBuildId}] => [ID ${currentBuildId}]`);
        ...
        if (activateFix && activateFix.toLowerCase() === "true") {
            ...
            let baseBuild = await buildApi.getBuild(compareBuildId);
            // There is only a workaround for Git but not for TFVC :(
            if (baseBuild.repository.type === "TfsGit") {
                let currentBuild = await buildApi.getBuild(currentBuildId);
                ...
                commits = await buildApi.getChangesBetweenBuilds(teamProject, compareBuildId, currentBuildId, 5000);
                workitems = await buildApi.getWorkItemsBetweenBuilds(teamProject, compareBuildId, currentBuildId, 5000);
            }
        } else {
...
async function run(): Promise<number> {
    var promise = new Promise<number>(async (resolve, reject) => {
        ...
        try {
            ...
            var deploymentBuildTag = tl.getInput("buildReleaseTag");
            if (delimiter === null) {
            ...
            if (tl.getVariable("Release.ReleaseId") === undefined) {
                ...
                if (deploymentBuildTag) {
                    const branchName = tl.getVariable("Build.SourceBranchName");
                    const mostRecentReleaseBuilds = await buildApi.getBuilds(teamProject, undefined, undefined, undefined, undefined,
                        undefined, undefined, undefined, undefined, BuildResult.Succeeded, [deploymentBuildTag], undefined, 1,
                        undefined, 1, undefined, BuildQueryOrder.QueueTimeDescending, branchName, undefined, undefined, undefined);

                    if (mostRecentReleaseBuilds.length) {
                        const { commits, workitems } = await getChangeset(vsts, buildApi, teamProject, buildId, mostRecentReleaseBuilds[0].id);
                    ...
                    if ((showOnlyPrimary === false) || (showOnlyPrimary === true && artifactInThisRelease.isPrimary === true)) {
                        if (arifactsInMostRecentRelease.length > 0) {
                            if (artifactInThisRelease.artifactType === "Build") {
                                agentApi.logInfo(`Looking for the [${artifactInThisRelease.artifactAlias}] with build number [${artifactInThisRelease.buildNumber}] in the most recent successful release [${mostRecentSuccessfulDeploymentName}]`);
                                for (var artifactInMostRecentRelease of arifactsInMostRecentRelease) {
                                    if (artifactInThisRelease.artifactAlias.toLowerCase() === artifactInMostRecentRelease.artifactAlias.toLowerCase()) {
                                        ...
                                        const chagneSet = await getChangeset(vsts, buildApi, teamProject,
                                            parseInt(artifactInThisRelease.buildId),
                                            isInitialRelease ? undefined : parseInt(artifactInMostRecentRelease.buildId));
            ...
            if (!deploymentBuildTag && buildId) {
                await buildApi.addBuildTag(teamProject, buildId, deploymentBuildTag);
            }
